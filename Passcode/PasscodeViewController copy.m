...
@property (nonatomic, assign) NSString *passcode;
@property (nonatomic, assign) NSString *passcodeOld;
@property (nonatomic, assign) NSString *passcodeCheck;
...
- (void)viewDidLoad {
    ...
    self.btnMarginTop = 60;
    self.btnMargin = 20;
    self.labelMarginTop = 50;
    self.labelMargin = 10;
    self.dotMarginTop = 30;
    self.passcode = @"";
    ...
    if ([type isEqualToString:@"passcodeCreate"]) {
        self.btnID.hidden = YES;
    }else if([type isEqualToString:@"passcodeVerify"]){
...
//        dispatch_time_t delayTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(.35 /*延迟执行时间*/ * NSEC_PER_SEC));
//        dispatch_after(delayTime, dispatch_get_main_queue(), ^{
//            if (self.isCreate && self.passcodeCheck.length == 0) {
//                self.dot1View.backgroundColor = COLOR_DOT_NORMAL;
//                self.dot2View.backgroundColor = COLOR_DOT_NORMAL;
//                self.dot3View.backgroundColor = COLOR_DOT_NORMAL;
//                self.dot4View.backgroundColor = COLOR_DOT_NORMAL;
...
//
//                } completion:nil];
//            }else if (self.isCreate && [self.passcode isEqualToString:self.passcodeCheck] && self.passcodeCheck.length == 4) {
//                NSLog(@"密码是%@",self.passcode);
//                NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
//                [userDefaults setObject:self.passcode  forKey:@"passcode"];
//                [userDefaults synchronize];
...
//
//                } completion:nil];
//            }else if(!self.isCreate && [self.passcode isEqualToString:self.passcodeOld]){
//                NSLog(@"密码是%@",self.passcode);
//                NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
//                [userDefaults setObject:self.passcode  forKey:@"passcode"];
//                [userDefaults synchronize];
...
- (void)btnDeleteAction:(UIButton*)btnEdit
{
    ...
    if (self.passcode.length == 0) {
        self.btnDelete.hidden = YES;
        if (isCreate && self.passcodeCheck.length != 4) {
            self.btnCancel.hidden = NO;
        }else if (isVerfiy) {
            self.btnCancel.hidden = YES;
        }
...
- (void)passwordDidCreate{
    
    
}

- (void)passwordDidVerify{
